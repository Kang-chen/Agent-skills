#!/usr/bin/env python3
"""Install a skill from a GitHub repo path into ~/.ai-skills/."""
...
def _tmp_root() -> str:
    base = os.path.join(tempfile.gettempdir(), "ai-skills")
    os.makedirs(base, exist_ok=True)
    return base


def _request(url: str) -> bytes:
    return github_request(url, "ai-skill-install")
...
def _prepare_repo(source: Source, method: str, tmp_dir: str) -> str:
    # Try git first (GitHub no longer supports SVN)
...
def _parse_args(argv: list[str]) -> Args:
    ...
    parser.add_argument("--dest", help=f"Destination skills directory (default: {_default_dest()})")
    parser.add_argument(
...
def main(argv: list[str]) -> int:
    ...
    try:
        ...
        print(f"\nNext steps:")
        print(f"  1. Run 'skills-sync' to sync to all IDEs")
        print(f"  2. Restart your IDE to load new skills")
